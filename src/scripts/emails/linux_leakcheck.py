...
def search_email_private(email, api_key):
    """Recherche un email en utilisant l'API privée LeakCheck (nécessite un plan payant)"""
    try:
        ...
        try:
            result = api.lookup(query=email, query_type="email", limit=100)
            return result
        except Exception as e:
            error_msg = str(e)
...
def search_email_public(email):
    """Recherche un email en utilisant l'API publique LeakCheck (gratuite)"""
    try:
        # Importer le module LeakCheck ici pour capturer les erreurs d'importation
        try:
...
if __name__ == "__main__":
    try:
        ...
        if len(sys.argv) != 3:
            print(json.dumps({"error": "Usage: python linux_leakcheck.py <email> <api_key>"}))
            sys.exit(1)
        ...
        result = search_email_private(email, api_key)
        ...
        if isinstance(result, dict) and result.get("error") and result.get("try_public"):
            ...
            result = search_email_public(email)
